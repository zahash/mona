use typegen_java::codegen;
use typegen_test_macro::fixture;
use typegen_test_utils::{Docker, Mount, Template, TestConfig};
...
static TEST_CONFIG: LazyLock<TestConfig> = LazyLock::new(|| {
    ...
    TestConfig {
        template: Template {
            ...
            codegen_output: PathBuf::from("src").join("TypeGen.java"),
        },
        docker: Docker {
...
async fn java_test<P: AsRef<Path>>(input_filepath: P) {
    typegen_test_utils::test(&TEST_CONFIG, codegen, &input_filepath).await;
}
